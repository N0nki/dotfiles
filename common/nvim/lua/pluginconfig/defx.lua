...
function defx_explorer_mode()
    vim.fn["defx#util#call_defx"]("Defx", '-new -auto-cd -split=vertical -winwidth=35 -direction=topleft -columns=indent:git:icons:space:filename -search=`expand("%:p")` `expand("%:p:h")`')
end

function defx_floating_mode()
    vim.fn["defx#util#call_defx"]("Defx", '-new -auto-cd -split=floating -columns=indent:git:icons:space:filename:size:type:time -search=`expand("%:p")` `expand("%:p:h")`')
end

function defx_detail_mode()
    vim.fn["defx#util#call_defx"]("Defx", '-new -auto-cd -columns=indent:git:icons:space:filename:size:type:time -search=`expand("%:p")` `expand("%:p:h")`')
end
