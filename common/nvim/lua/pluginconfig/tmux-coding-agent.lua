...
vim.keymap.set(
  'n',
  '<leader>tr',
  tmux_ai.send_with_prompt('このコードをレビューしてください。'),
  vim.tbl_extend('force', opts, { desc = 'Send buffer for review (auto-detect)' })
)
...
vim.keymap.set(
  ...
  vim.tbl_extend('force', opts, { desc = 'Send buffer for refactoring (auto-detect)' })
)
...
vim.keymap.set(
  'n',
  '<leader>te',
  tmux_ai.send_with_prompt('このコードを説明してください。'),
  vim.tbl_extend('force', opts, { desc = 'Send buffer for explanation (auto-detect)' })
)
